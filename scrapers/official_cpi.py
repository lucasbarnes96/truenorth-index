from __future__ import annotations

import csv
import io
import urllib.request
import zipfile
from typing import Any

from .common import USER_AGENT

STATCAN_CPI_ZIP = "https://www150.statcan.gc.ca/n1/en/tbl/csv/18100004-eng.zip"
ALL_ITEMS = "All-items"


def _download_zip_bytes(url: str) -> bytes:
    req = urllib.request.Request(url, headers={"User-Agent": USER_AGENT})
    with urllib.request.urlopen(req, timeout=30) as response:
        return response.read()


def _load_cpi_rows() -> list[dict[str, str]]:
    data = _download_zip_bytes(STATCAN_CPI_ZIP)
    if not zipfile.is_zipfile(io.BytesIO(data)):
        return []

    with zipfile.ZipFile(io.BytesIO(data)) as zf:
        csv_name = next(name for name in zf.namelist() if name.endswith(".csv"))
        with zf.open(csv_name) as handle:
            decoded = io.TextIOWrapper(handle, encoding="utf-8-sig", errors="ignore")
            return list(csv.DictReader(decoded))


def _candidate_rows(rows: list[dict[str, str]]) -> list[dict[str, str]]:
    candidates = [
        r
        for r in rows
        if r.get("GEO") == "Canada"
        and (r.get("Products and product groups") or "").strip() == ALL_ITEMS
        and r.get("VALUE")
        and r.get("REF_DATE")
    ]
    candidates.sort(key=lambda r: r["REF_DATE"])
    return candidates


def fetch_official_cpi_series() -> list[dict[str, Any]]:
    try:
        rows = _candidate_rows(_load_cpi_rows())
        if len(rows) < 2:
            return []

        series: list[dict[str, Any]] = []
        for idx, row in enumerate(rows):
            current = float(row["VALUE"])
            prev = float(rows[idx - 1]["VALUE"]) if idx >= 1 else None
            prev_year = float(rows[idx - 12]["VALUE"]) if idx >= 12 else None

            mom = ((current / prev) - 1) * 100 if prev else None
            yoy = ((current / prev_year) - 1) * 100 if prev_year else None

            series.append(
                {
                    "ref_date": row["REF_DATE"],
                    "index_value": round(current, 4),
                    "mom_pct": round(mom, 3) if mom is not None else None,
                    "yoy_pct": round(yoy, 3) if yoy is not None else None,
                }
            )
        return series
    except Exception:  # pragma: no cover - network dependent
        return []


def fetch_official_cpi_summary() -> dict:
    try:
        series = fetch_official_cpi_series()
        if len(series) < 13:
            return {"latest_release_month": None, "mom_pct": None, "yoy_pct": None}

        latest = series[-1]
        return {
            "latest_release_month": latest["ref_date"],
            "mom_pct": latest["mom_pct"],
            "yoy_pct": latest["yoy_pct"],
        }
    except Exception:  # pragma: no cover - network dependent
        return {"latest_release_month": None, "mom_pct": None, "yoy_pct": None}
