from __future__ import annotations

from datetime import datetime, timezone

from .common import fetch_url, parse_floats_from_text, utc_now_iso
from .types import Quote, SourceHealth

CAA_GAS_URL = "https://www.caa.ca/gas-prices/"


def scrape_transport() -> tuple[list[Quote], list[SourceHealth]]:
    quotes: list[Quote] = []
    health: list[SourceHealth] = []

    try:
        html = fetch_url(CAA_GAS_URL)
        values = [v for v in parse_floats_from_text(html) if 80 <= v <= 260]
        values = values[:40]
        observed = datetime.now(timezone.utc).date()
        for i, value in enumerate(values):
            quotes.append(
                Quote(
                    category="transport",
                    item_id=f"gas_observation_{i}",
                    value=value,
                    observed_at=observed,
                    source="caa_scrape",
                )
            )

        health.append(
            SourceHealth(
                source="caa_scrape",
                category="transport",
                tier=2,
                status="fresh" if quotes else "missing",
                last_success_timestamp=utc_now_iso() if quotes else None,
                detail=f"Collected {len(quotes)} gas observations.",
            )
        )
    except Exception as err:  # pragma: no cover - network dependent
        health.append(
            SourceHealth(
                source="caa_scrape",
                category="transport",
                tier=2,
                status="missing",
                last_success_timestamp=None,
                detail=f"Fetch failed: {err}",
            )
        )

    return quotes, health
